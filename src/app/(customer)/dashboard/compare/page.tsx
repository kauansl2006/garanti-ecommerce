...
<table>
    <tbody>
        {informations.map((information) => (
            <tr key={information.key} className="even:bg-neutral-200">
                ...
                {products.map((product, index) => (
                    <td key={index} className="max-w-[290px] w-full border-x-[1px] border-neutral-300 border-collapse px-[22px] py-[12px]">
                        {information.key === "cardProduto" && (
                            <div className="flex flex-col items-center justify-center gap-[16px]">
                                ...
                                <div className="size-[172px] relative">
                                    <Image
                                ...
                                <p className="text-b-sm leading-b-sm font-normal line-clamp-2 text-ellipsis">
                                    {product.title}
                                </p>   
                                <div className="w-full flex gap-[12px]">
                                    <Button                                         
                                        className="cursor-pointer flex gap-[8px] w-[140px] h-[44px] text-white text-[14px]/[48px] font-bold rounded-[4px]"
                                    >
                                        ADQUIRIR
                                    ...
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        className="cursor-pointer size-[44px] rounded-[4px] bg-white hover:bg-primary text-primary hover:text-white p-[12px]"
                                    >
                        ...
                        {information.key === "feedbackCliente" && product.score && (
                            <Rating score={product.score} />
                        )}
                        {information.key === "preco" && (
                            <p className="text-b-sm leading-b-sm font-normal">{product.price}</p>
                        )}
                        {information.key === "vendidoPor" && (
                            <p className="text-b-sm leading-b-sm font-normal">{product.store}</p>
                        )}
                        {information.key === "marca" && (
                            <p className="text-b-sm leading-b-sm font-normal">{product.brand}</p>
                        )}
                        {information.key === "modelo" && (
                            <p className="text-b-sm leading-b-sm font-normal">{product.model}</p>
                        )}
                        {information.key === "statusEstoque" && (
                            <p className="text-b-sm leading-b-sm font-normal">{product.stock}</p>
                        )}
                        {information.key === "tamanho" && (
                            <p className="text-b-sm leading-b-sm font-normal">{product.size}</p>
                        )}
                        {information.key === "peso" && (
                            <p className="text-b-sm leading-b-sm font-normal">{product.weight}</p>
                        )}
                    </td>
                ))}
            </tr>
